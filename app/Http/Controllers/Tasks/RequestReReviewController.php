...
class RequestReReviewController extends Controller
{
    public function __invoke(YakTask $task, AppService $github, EnqueuePrReview $enqueue): RedirectResponse
    {
        ...
        $prNumber = $task->mode === TaskMode::Review
            ? (int) ($context['pr_number'] ?? 0)
            : (int) $task->pr_number;
        ...
        if ($newTask !== null) {
            return redirect()->route('tasks.show', $newTask)->with('success', $task->mode === TaskMode::Review
                ? 'Re-review requested. A fresh review is now running for this PR -- this page updates live as it progresses.'
                : 'Review requested. Yak is now reviewing this PR -- this page updates live as it progresses.');
        }
