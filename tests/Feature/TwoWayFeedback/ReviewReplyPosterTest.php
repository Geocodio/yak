...
it('quotes only the comment id when the original line is not in the task description', function () {
    ...
    $github->shouldReceive('commentOnPullRequest')->once()->withArgs(fn (int $i, string $r, int $n, string $body): bool => str_contains($body, 'review comment 555') && str_contains($body, 'Answer.'));
