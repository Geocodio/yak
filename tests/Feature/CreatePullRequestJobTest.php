...
test('PR body includes warning footer', function () {
    Http::fake([
        'api.github.com/app/installations/*/access_tokens' => Http::response([
            'token' => 'ghs_test',
            'expires_at' => now()->addHour()->toIso8601String(),
    ...
    Http::assertSent(function ($request) {
        return str_contains($request->url(), '/pulls')
            && str_contains($request['body'], 'This PR was generated by Yak. Review before merging.');
    });
});

test('PR body includes files changed list', function () {
    Http::fake([
        'api.github.com/app/installations/*/access_tokens' => Http::response([
            'token' => 'ghs_test',
            'expires_at' => now()->addHour()->toIso8601String(),
    ...
    $task = YakTask::factory()->awaitingCi()->create([
        'repo' => 'org/my-repo',
        'branch_name' => 'yak/FIX-FILES',
        'source' => 'manual',
        'description' => 'Test files list',
        'attempts' => 1,
    ]);
    ...
    Http::assertSent(function ($request) {
        ...
        return str_contains($body, '### Files changed')
            && str_contains($body, '`app/Models/User.php`')
            && str_contains($body, '`app/Http/Controllers/AuthController.php`')
            && str_contains($body, '`tests/Feature/AuthTest.php`');
    });
});
