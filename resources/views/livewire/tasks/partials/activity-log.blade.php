    ...
    <div class="relative">
        ...
        @foreach($this->groupedLogs as $entry)
            @if($entry['type'] === 'group')
                ...
                >
                    <button wire:click="toggleGroup({{ $groupIndex }})" class="flex w-full items-center gap-2 px-3 py-2 text-left transition-colors hover:bg-[rgba(245,240,232,0.5)]">
                        ...
                        <span class="min-w-0 flex-1 truncate text-xs italic text-yak-blue">{{ $lastLog->message }}</span>
                    </button>
                    @if($isGroupExpanded)
                        <div class="border-t border-[rgba(200,184,154,0.25)] bg-[rgba(245,240,232,0.3)]">
                            @foreach($entry['logs'] as $groupLog)
                                <div class="border-b border-[rgba(200,184,154,0.15)] px-3 py-1.5 last:border-b-0">
                                    <span class="text-xs italic text-yak-blue">{{ $groupLog->message }}</span>
                                </div>
                            @endforeach
                        </div>
                    @endif
            ...
            @else
                ...
                >
                    <button
                        @if($hasExpandableContent && !$isMilestone) wire:click="openLogDrawer({{ $index }})" @endif
                    ...
                    >
                        ...
                        @endif
                        <span class="min-w-0 flex-1 truncate text-xs {{ $isAssistant ? 'italic text-yak-blue' : 'text-yak-slate' }} {{ $isMilestone ? 'font-semibold' : '' }}">{{ $log->message }}</span>
