...
6. VISUAL CAPTURE: When the task involves UI changes and a dev URL is available, take screenshots and a short screen recording after your changes to verify visual correctness. Save all screenshots and recordings to the `.yak-artifacts/` directory in the repo root (create it if it doesn't exist). Use descriptive filenames like `homepage-after.png` or `confetti-demo.webm`. These will be attached to the PR automatically. IMPORTANT: Always stop any dev servers (npm, gatsby, vite, etc.) you started before finishing your work. Background processes prevent the task from completing.
