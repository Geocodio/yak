...
- `checkout_refresh` — same rule: `cd /workspace && …` before any compose/composer/npm/artisan commands. Runs after `git fetch && git checkout $sha` (which Yak has already done for you from `/workspace`). Empty string is only correct if a plain checkout is enough (no build, no migration, no asset compile).
